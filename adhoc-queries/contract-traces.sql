...
select table_type, table_schema
    ...
    , right(table_name,
        length(table_name) 
        - 2 -- subtract the 2 underscores we're excluding
        - length(split_part(table_name,'_',1)) -- subtract the contract_name
        - length(split_part(table_name,'_',2)) -- subtract the trace_type
