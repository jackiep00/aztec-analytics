...
case proofId -- deposit, withdraw
 ...
 -- currently hardcoded here but should be able to fetch dynamically from aztec_v2.RollupProcessor_call_getSupportedAsset
 case assetId
   when 0
   then 
     asset = 'Ethereum';
...
if proofId = 3 then
  proofType = 'SEND';
elsif assetId = 4 then
  proofType = 'ACCOUNT';
elsif assetId = 5 then
...
select
  ...
  --   publicOwner bytea,
  EMPTY_BYTES_32,
  --   asset text - NOT VISIBLE in this transaction types
  null 
 into innerProof;
